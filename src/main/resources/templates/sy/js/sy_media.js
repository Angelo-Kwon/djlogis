let sy_media = {
	...
	fnEvent: function() {
		...
		$("#btnDel").on("click", function() {
			const checkList = sy_media.grid.Checkbox('CHK').getCheckedNodes();
	       	if(!checkList.length) {
	       		alert('선택된 ROW가 없습니다.');
	       		return;
	       	}
	   		...
	   		if(confirm(checkList.length + '행을 삭제하시겠습니까?')) {
	  			for(const checkData of checkList) {
	  				sy_media.grid.deleteRow({ rowIndx: checkData.pq_ri });
	  	   		}
		...
		$("#btnSave_e").on("click", function() {
			let val_check = false;
			val_check = sy_media.fnNewNoticeValueCheck();
			if(val_check) {
				sy_media.fnSaveBoard();
			}
		});
		...
		$(document).on('change', '.open-file', function(event) {
		    sy_media.selectFile(event.currentTarget);
		});
		
		//  첨부파일 삭제 버튼
		$(document).on('click', '[id^=removeBtn]', function(event) {
		    sy_media.removeFile(event.currentTarget);
		});
		
		// TOAST UI EDITOR 세팅
	    sy_media.editor = new toastui.Editor({
	        el: document.querySelector('#editor'), // 에디터를 적용할 요소 (컨테이너)
	        height: '533px',                       // 에디터 영역의 높이 값 (OOOpx || auto)
	        initialEditType: 'wysiwyg',            // 최초로 보여줄 에디터 타입 (markdown || wysiwyg)
	    });
	    
	    sy_media.setLabelCss();
		...
		$("#btnEdit").on("click", function() {
			...
			$('#file1_input').val('');
			sy_media.fnDetail(sy_media.boardSeq);
		});
	...
	setGrid: function() {
		...
		let options = { 
			...
			cellClick: function(e, ui) {
				if(ui.dataIndx != 'USE_YN' && ui.dataIndx != 'CHK'){
					sy_media.fnMoveNoticeReg(ui.rowData.BOARD_SEQ);						
				} 
			}
		};
		...
		gridCmmn.open();
		sy_media.grid = gridCmmn.getGrid();
	},
	...
	fnSearch: function() {
		...
		$.ajax({
			...
			success: function(data, status, res) {
				...
				//데이터 ADD
				sy_media.gridDataList = data.dataList;
				sy_media.grid.option("dataModel.data", data.dataList);
				sy_media.grid.refreshDataAndView();
				
				// 사용여부 수정 가능
				sy_media.grid.colModel[4].editable = true;
	
				sy_media.changes = sy_media.grid.getChanges();
			},
			complete: function(e) {
				sy_media.grid.hideLoading();
			},
			error: function(e) {
				alert(e);
			}
	 ...
	 */
	 fnSave: function() {
		...
		$.ajax({
			...
			//로딩바 SHOW
			beforeSend: function() {
				sy_media.grid.option("strLoading", "Loding...")
		        sy_media.grid.showLoading();
		    },
			success:function(data, status, res){
				//오류확인
	...
	fnDetail: function(seq) {
		...
		$.ajax({
			...
			success:function(data, status, res){
				...
				sy_media.info = data.dataInfo;
				
				// TOAST UI EDITOR에 세팅
				sy_media.editor.setHTML(data.dataInfo?.CONTENTS);
	...
	fnSaveBoard() {
		...
		param.set("boardId", 3);
		param.set("schContent", sy_media.editor.getHTML());
		param.set("schUseYn", sy_media.info.USE_YN ? sy_media.info.USE_YN : 'Y');
		
		if(sy_media.boardSeq != ''){
			param.set("schBoardSeq", sy_media.info.BOARD_SEQ);
			param.set("schBoardId", sy_media.boardSeq);		
		}
	...
	fnNewNoticeValueCheck: function() {
		...
		if($("#schNoticsYn_e").val() == "") {
			...
			result = false;
			return false;
		} else if(sy_media.editor.getHTML() == "<p><br></p>") {
	...
	// grid 초기화
	fnGridInit: function() {
		sy_media.gridDataList = null;
		sy_media.grid.option("dataModel.data", []);
		sy_media.grid.refreshDataAndView();
	}
}
