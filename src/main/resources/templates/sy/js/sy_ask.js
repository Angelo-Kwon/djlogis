let sy_ask = {
	...
	fnEvent: function() {
		...
		$("#btnDel").on("click", function() {
			const checkList = sy_ask.grid.Checkbox('CHK').getCheckedNodes();
	       	if(!checkList.length) {
	       		alert('선택된 ROW가 없습니다.');
	       		return;
	       	}
	   		...
	   		if(confirm(checkList.length + '행을 삭제하시겠습니까?')) {
	  			for(const checkData of checkList) {
	  				sy_ask.grid.deleteRow({ rowIndx: checkData.pq_ri });
	  	   		}
		...
		$("#btnSave_e").on("click", function() {
			let val_check = false;
			val_check = sy_ask.fnNewNoticeValueCheck();
			if(val_check) {
				sy_ask.fnSaveBoard();
			}
		});
		...
		$(document).on('change', '.open-file', function(event) {
		    sy_ask.selectFile(event.currentTarget);
		});
		
		//  첨부파일 삭제 버튼
		$(document).on('click', '[id^=removeBtn]', function(event) {
		    sy_ask.removeFile(event.currentTarget);
		});
		
		// TOAST UI EDITOR 세팅
	    sy_ask.editor = new toastui.Editor({
		...
		$("#btnEdit").on("click", function() {
			...
			$('#file1_input').val('');
			sy_ask.fnDetail(sy_ask.boardSeq);
		});
		...
		let options = { 
			...
			cellClick: function(e, ui) {
				if(ui.dataIndx != 'USE_YN' && ui.dataIndx != 'CHK'){
					sy_ask.fnMoveNoticeReg(ui.rowData.BOARD_SEQ);						
				} 
			}
		};
	...
	fnSearch: function() {
		...
		$.ajax({
			...
			success: function(data, status, res) {
				...
				//데이터 ADD
				sy_ask.gridDataList = data.dataList;
				sy_ask.grid.option("dataModel.data", data.dataList);
				sy_ask.grid.refreshDataAndView();
				
				// 사용여부 수정 가능
				sy_ask.grid.colModel[4].editable = true;
	
				sy_ask.changes = sy_ask.grid.getChanges();
			},
			complete: function(e) {
				sy_ask.grid.hideLoading();
			},
			error: function(e) {
				alert(e);
			}
	 ...
	 */
	 fnSave: function() {
		...
		$.ajax({
			...
			//로딩바 SHOW
			beforeSend: function() {
				sy_ask.grid.option("strLoading", "Loding...")
		        sy_ask.grid.showLoading();
		    },
			success:function(data, status, res){
				//오류확인
	...
	fnDetail: function(seq) {
		...
		$.ajax({
			...
			success:function(data, status, res){
				...
				sy_ask.info = data.dataInfo;
				
				// TOAST UI EDITOR에 세팅
				sy_ask.editor.setHTML(data.dataInfo?.CONTENTS);
				$("#viewer").html(data.dataInfo?.CONTENTS);

				if($('.notice-edit').is(':visible')){

					// 제목, 작성자명, 작성일시, 공지여부, 사용여부 세팅
					$("#schUserNm_e").val(data.dataInfo?.WRITER);
					$("#schTitle_e").val(data.dataInfo?.TITLE);
					//$("#schUseYn_e").val(data.dataInfo?.USE_YN);
					$("#schHitCnt_e").val(data.dataInfo?.HIT_CNT);
					$("#schCreDt_e").val(common.getToday(5));
					$("#schPhone").val(data.dataInfo?.PHONE);

					common.setCommCode($("#schNoticsYn_e"),"NOTICS_YN", data.dataInfo?.NOTICS_YN, 3); // //여부
					...
					$("#schUserNm_d").text(data.dataInfo?.WRITER);
					...
					$("#schPhone").text(data.dataInfo?.PHONE);
	...
	fnSaveBoard() {
		...
		param.set("boardId", 4);
		param.set("schContent", sy_ask.editor.getHTML());
		//param.set("schUseYn", sy_ask.info.USE_YN ? sy_ask.info.USE_YN : 'Y');
		
		if(sy_ask.boardSeq != ''){
			param.set("schBoardSeq", sy_ask.info.BOARD_SEQ);
			param.set("schBoardId", sy_ask.boardSeq);		
		}
	...
	fnNewNoticeValueCheck: function() {
		...
		if($("#schNoticsYn_e").val() == "") {
			...
			result = false;
			return false;
		} else if(sy_ask.editor.getHTML() == "<p><br></p>") {
	...
	// grid 초기화
	fnGridInit: function() {
		sy_ask.gridDataList = null;
		sy_ask.grid.option("dataModel.data", []);
		sy_ask.grid.refreshDataAndView();
	}
}
